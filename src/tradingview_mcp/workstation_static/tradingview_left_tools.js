(function installTradingViewLeftTools() {
  const TOOL_GROUPS = [
    { id: 'cursor', icon: '⌖', label: 'Cursor tools', items: [
    ...
    { id: 'lines', icon: '╱', label: 'Lines', items: [
    ...
    { id: 'channels', icon: '≋', label: 'Channels', items: [
    ...
    { id: 'pitchforks', icon: '⋔', label: 'Pitchforks', items: [
    ...
    { id: 'fib-gann', icon: '⑂', label: 'Gann and Fibonacci', items: [
    ...
    { id: 'geometry', icon: '□', label: 'Geometry', items: [
    ...
    { id: 'annotation', icon: 'T', label: 'Text and notes', items: [
    ...
    { id: 'measure', icon: '⌁', label: 'Measure and zoom', items: [
    ...
    { id: 'workspace', icon: '☆', label: 'Workspace', items: [
  ...
  function getLeftPanelRightEdge(centerRect) {
    ...
    const selector = 'main > aside:not(.right), aside:not(.right):not(.tradingview-right-panel)';
    return Array.from(document.querySelectorAll(selector)).reduce((rightEdge, panel) => {
  ...
  function syncToolbarGeometry() {
    ...
    rootStyle.setProperty('--tv-left-toolbar-left', `${Math.round(Math.max(0, getLeftPanelRightEdge(centerRect)))}px`);
    rootStyle.setProperty('--tv-left-toolbar-top', `${Math.round(Math.max(0, centerRect.top))}px`);
    rootStyle.setProperty('--tv-left-toolbar-bottom', `${Math.round(Math.max(0, window.innerHeight - centerRect.bottom))}px`);
  }
  ...
  function clickAction(action) {
    ...
    if (action === 'watchlist') {
      if (typeof window.setTradingViewWatchlistPanelOpen === 'function') window.setTradingViewWatchlistPanelOpen(!document.body.classList.contains('watchlist-expanded'));
      else document.body.classList.toggle('watchlist-expanded');
      syncToolbarGeometry();
      return true;
    }
    if (action === 'research') {
      if (typeof window.setTradingViewRightPanelOpen === 'function') window.setTradingViewRightPanelOpen(!document.body.classList.contains('research-expanded'));
      else document.body.classList.toggle('research-expanded');
      syncToolbarGeometry();
      return true;
    }
    if (action === 'top-tools') {
